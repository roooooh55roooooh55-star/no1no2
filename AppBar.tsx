...
const AppBar: React.FC<AppBarProps> = ({ onViewChange, onRefresh, currentView }) => {
  ...
  const getBtnClass = (view: AppView, colorClass: string, glowColor: string) => {
    ...
    return `w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-500 border active:scale-95 ${
      isActive 
      ? `bg-black/60 ${colorClass} border-current scale-110 z-20 shadow-[0_0_20px_${glowColor}]` 
      : `bg-white/5 border-white/10 ${colorClass} opacity-70`
    } hover:opacity-100`;
  };

  return (
    <header className="fixed top-0 left-0 right-0 h-16 bg-black/80 backdrop-blur-3xl z-[100] border-b border-white/5 px-2 flex items-center justify-between shadow-2xl">
      ...
      >
        ...
        <img src="https://i.top4top.io/p_3643ksmii1.jpg" className="w-14 h-14 rounded-full border-2 border-red-500 relative z-10 shadow-[0_0_20px_red]" alt="Logo" />
      </button>
      ...
      <div className="flex items-center gap-1">
        ...
        <button 
          ...
          className="w-8 h-8 rounded-lg bg-white/5 border border-red-600 flex items-center justify-center text-red-600 opacity-80"
        >
