...
const AppBar: React.FC<AppBarProps> = ({ onViewChange, onRefresh, currentView }) => {
  ...
  const getBtnClass = (view: AppView, colorClass: string, glowColor: string) => {
    ...
    return `w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-500 border-2 active:scale-95 ${
      isActive 
      ? `bg-black/60 ${colorClass} border-current scale-110 z-20 shadow-[0_0_30px_${glowColor}]` 
      : `bg-white/5 border-white/20 ${colorClass} opacity-80`
    } hover:opacity-100`;
  };

  return (
    <header className="fixed top-0 left-0 right-0 h-20 bg-black/80 backdrop-blur-3xl z-[100] border-b border-white/5 px-2 flex items-center justify-between shadow-2xl">
      ...
      <button 
        ...
        className="relative group active:scale-95 transition-all duration-500 mx-3"
      >
        <div className="absolute inset-0 bg-red-600/30 rounded-full blur-xl animate-pulse group-hover:bg-red-600/50"></div>
        <img src="https://i.top4top.io/p_3643ksmii1.jpg" className="w-16 h-16 rounded-full border-2 border-red-500 relative z-10 shadow-[0_0_20px_red]" alt="Logo" />
      </button>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button 
          ...
          className="w-10 h-10 rounded-xl bg-white/5 border-2 border-red-600 flex items-center justify-center text-red-600 opacity-80"
        >
