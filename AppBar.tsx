...
const AppBar: React.FC<AppBarProps> = ({ onViewChange, onRefresh, currentView }) => {
  ...
  const getBtnClass = (view: AppView, colorClass: string, glowColor: string) => {
    ...
    return `w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-500 border-2 active:scale-95 ${
      isActive 
      ? `bg-black/60 ${colorClass} border-current scale-110 z-20 shadow-[0_0_30px_${glowColor},0_0_60px_${glowColor}99]` 
      : `bg-white/5 border-white/20 ${colorClass} opacity-80 shadow-[0_0_10px_${glowColor}66]`
    } hover:shadow-[0_0_20px_${glowColor}] hover:opacity-100`;
  };

  return (
    <header className="fixed top-0 left-0 right-0 h-20 bg-black/80 backdrop-blur-3xl z-[100] border-b border-white/5 px-3 flex items-center justify-between shadow-[0_15px_50px_rgba(0,0,0,0.9)]">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button 
          ...
          className="w-9 h-9 rounded-xl bg-white/5 border-2 border-red-600 flex items-center justify-center text-red-600 opacity-90 shadow-[0_0_15px_rgba(220,38,38,0.5)] hover:opacity-100 hover:shadow-[0_0_30px_rgba(220,38,38,0.8)] transition-all"
        >
